...
class CQN(nn.Module):
    ...
    def __init__(self,
        ...
        if self.use_automatic_entropy_tuning:
            ...
            self.alpha_optimizer = torch.optim.Adam(params=self.log_alpha.parameters(), lr=critic_lr)
        else:
            self.log_alpha = None

        if self.cql_lagrange:
            self.log_alpha_prime = Scalar(1.0)
            self.alpha_prime_optimizer = torch.optim.Adam(params=self.log_alpha_prime.parameters(), lr=critic_lr)
    ...
    # reward함수
    def train(self, state, action, reward, next_states, dones,bc=True):
        # logits = self.actor(state)
        ...
        if bc:
            logits = self.actor(state)
            log_probs = Categorical(logits=logits).log_prob(action.squeeze()).view(-1, 1)
